...
subroutine getNcid(self)
	...
	self%status = NF90_open(trim(self%header%file_name%name), NF90_NOWRITE, self%nc_id)
	call self%check()
...
do i=1,self%dim_size
	...
	self%status = NF90_inq_dimid(self%nc_id, trim(self%header%dim_name(i)%name), self%dim_id(i))
	call self%check()

	self%status = nf90_inquire_dimension(self%nc_id, self%dim_id(i), len=self%dim_len(i))
	call self%check()

	self%status = nf90_inq_varid(self%nc_id, trim(self%header%dim_name(i)%name), self%dim_id(i))
	call self%check()

	allocate(self%dim_data(i)%vector(self%dim_len(i)))

	print*,'Dimension: ',trim(self%header%dim_name(i)%name)
...
self%status = NF90_inq_varid(self%nc_id, trim(self%header%var_name(i)%name), self%var_id(i))
call self%check()

allocate(self%var_data(i)%data(self%dim_len(1),self%dim_len(2),self%dim_len(3),self%dim_len(4)))

	print*,'Field: ',trim(self%header%var_name(i)%name)
