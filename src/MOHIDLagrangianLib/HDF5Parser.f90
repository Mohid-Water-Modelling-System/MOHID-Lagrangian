    ...
    do i=1, self%nVars !going trough all variables
        if (self%varData(i)%simName == varName ) then   !found the requested var
            ...
            if(self%varData(i)%ndims == 3) then !3D variable
                ...
                do var = 1, self%nVars
                    if (self%varData(var)%simName == mapVarName) then
                        if (self%varData(var)%ndims == 3 .or. (self%varData(var)%ndims == 4 .and. self%varData(var)%isStaticInTime)) then
                            !map var is static(in time) = waterpoints or waterpoints3D
    ...
    do idx = 1, nmembersIn
        ...
        if     (obj_type == H5G_DATASET_F) then
            ...
            elseif(isStaticVariable) then
                !Adicionar aqui codigo para verificar se é dado waterpoints3D e meter rank+1
                if (Globals%Var%checkMappingVar(nameString)) then
                    !Must be WaterPoints or WaterPoints3D which do not vary in time
                    self%varData(VarCounter)%ndims = rank + 1
                    allocate(self%varData(VarCounter)%dimids(rank + 1))
                    self%varData(VarCounter)%isStaticInTime = .true.
                else
                    self%varData(VarCounter)%ndims = rank
                    allocate(self%varData(VarCounter)%dimids(rank))
                endif
                ...
                elseif (rank == 3 .and. Globals%Var%checkMappingVar(nameString)) then
                    self%varData(VarCounter)%dimids(1) = 1 !Lat
                    self%varData(VarCounter)%dimids(2) = 2 !Lon
