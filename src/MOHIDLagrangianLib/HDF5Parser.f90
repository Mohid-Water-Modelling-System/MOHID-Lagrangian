    ...
    type :: ncfile_class !< A class that models a hdf5 file
        type(string) :: filename   !< name of the file to read
        integer :: hdfID             !< ID of the file
    ...
    !> @brief
    !> Opens the hdf5 file, assigns the NCID and checks for errors
    !> @param[in] self
    !---------------------------------------------------------------------------
    subroutine getHdfid(self)
    class(ncfile_class), intent(inout) :: self
    call h5fopen_f (trim(self%filename%chars()), access_flags = H5F_ACC_RDONLY_F,               &
                    file_id = self%hdfID, HDFERR = self%status)
    
    call self%check()
    end subroutine getHdfid
    ...
    call h5gopen_f        (self%hdfID, trim(GroupNameIn), gr_idIn, STAT)
    ...
    !self%status = nf90_inquire(self%hdfID, self%nDims, self%nVars, self%nAtt, self%uDimID)
    ...
    !        end if
    !    else
    !        outext = '[hdf5parser::getNCDimMetadata]: ncfile '//trim(self%filename%chars())//' has less than 3 or more than 4 dimensions. Stopping'
    !        call Log%put(outext)
    !        stop
    !    end if
    !
