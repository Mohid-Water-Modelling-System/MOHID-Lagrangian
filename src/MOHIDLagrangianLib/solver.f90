    ...
    do bkg = 1, size(bdata)
        ...
        !update positions for the predictor step:  x_n + k1./2*dt
        ...
        !update positions for the predictor step: x_n + k2*dt/2
        ...
        aot%x = aot%x + (dt/6.)*(k(1)%u + 2.*k(2)%u + 2.*k(3)%u + k(4)%u)
        aot%y = aot%y + (dt/6.)*(k(1)%v + 2*.k(2)%v + 2.*k(3)%v + k(4)%v)
        aot%z = aot%z + (dt/6.)*(k(1)%w + 2.*k(2)%w + 2.*k(3)%w + k(4)%w)  
