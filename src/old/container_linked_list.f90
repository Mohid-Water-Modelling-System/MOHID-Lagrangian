    ...
    module container_linked_list
      ...
      type container_list
         private
         class(container_link), pointer :: firstLink => null() !> first link in list
         class(container_link), pointer :: lastLink => null()  !> last link in list
       contains
    ...
    contains
      ...
      !> a polymorphic container so it can be used for any data
      !
      !> @param[value]
      !---------------------------------------------------------------------------
      subroutine add(this, value)
        class(container_list) :: this
        class(*) :: value
        class(container_link), allocatable :: newLink
      
        if (.not. associated(this%firstLink)) then
           this%firstLink => container_link(value, this%firstLink)
           this%lastLink => this%firstLink
        else
           newLink = container_link(value, this%lastLink%nextLink())
      ...
      function firstValue(this)
        class(container_list) :: this
        class(*), allocatable :: firstValue
            allocate(firstValue, source=this%firstLink%getValue())      
