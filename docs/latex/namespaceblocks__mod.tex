...
295     \textcolor{keyword}{call }self%AoT%toTracers()
\end{DoxyCode}
...
306     \textcolor{keyword}{call }self%AoT%Clean()
\end{DoxyCode}
...
334     ix = min(int((pt%x + bbox%offset%x)/globals%SimDefs%blocksize%x) + 1, globals%SimDefs%numblocksx)
335     iy = min(int((pt%y + bbox%offset%y)/globals%SimDefs%blocksize%y) + 1, globals%SimDefs%numblocksy)
336     temp = 2*ix + iy -2
337     \textcolor{keywordflow}{if} (temp > globals%SimDefs%numblocks) \textcolor{keywordflow}{then}
...
101     self%extents%pt = templatebox%pt
102     self%extents%size = templatebox%size
...
107     \textcolor{keyword}{call }self%Integrator%initialize(i, globals%Parameters%IntegratorNames(i))
...
84     numalloctracers = self%LTracer%getSize()
\end{DoxyCode}
...
355     temp\_str = self%id
356     outext=\textcolor{stringliteral}{'-->Block '}//temp\_str//\textcolor{stringliteral}{' is a'}
357     \textcolor{keyword}{call }log%put(outext,.false.)
358     \textcolor{keyword}{call }geometry%print(self%extents)
359     temp\_str = self%LSource%getSize()
360     outext=\textcolor{stringliteral}{'      and has '}//temp\_str//\textcolor{stringliteral}{' Sources'}
361     \textcolor{keyword}{call }log%put(outext,.false.)
\end{DoxyCode}
...
375     temp\_str = self%id
376     outext=\textcolor{stringliteral}{'-->Block '}//temp\_str//\textcolor{stringliteral}{' is a'}
377     \textcolor{keyword}{call }log%put(outext,.false.)
378     \textcolor{keyword}{call }geometry%print(self%extents)
379     temp\_str = self%LSource%getSize()
380     outext=\textcolor{stringliteral}{'      and has '}//temp\_str//\textcolor{stringliteral}{' Sources'}
381     \textcolor{keyword}{call }log%put(outext,.false.)
382     \textcolor{keyword}{call }self%LSource%print()
\end{DoxyCode}
...
320     \textcolor{keyword}{call }dblock(blk)%LTracer%add(trc)
\end{DoxyCode}
Here is the caller graph for this function\+:\nopagebreak
\begin{figure}[H]
\begin{center}
...
393     \textcolor{keywordtype}{logical}, \textcolor{keywordtype}{intent(in)} ::  auto
394     \textcolor{keywordtype}{integer}, \textcolor{keywordtype}{intent(in)} ::  nblk
395     \textcolor{keywordtype}{integer}, \textcolor{keywordtype}{intent(out)} :: nxi, nyi
396     \textcolor{keywordtype}{type}(string) :: outext, temp(2)
397     \textcolor{keywordtype}{integer} :: i, j, b
398     \textcolor{keywordtype}{real(prec)} :: ar
399     \textcolor{keywordtype}{type}(box) :: tempbox
400 
401     \textcolor{keywordflow}{if} (auto) \textcolor{keywordflow}{then}
402         ar = bbox%size%x/bbox%size%y
403         ar = get\_closest\_twopow(ar) \textcolor{comment}{!aspect ratio of our bounding box}
404         nyi = sqrt(nblk/ar)
405         \textcolor{keywordflow}{if} (nyi == 0) \textcolor{keywordflow}{then}
406             temp(1) = ar
407             outext=\textcolor{stringliteral}{'[setBlocks]: block auto sizing failed. Bouding box aspect ratio = '}//temp(1)//\textcolor{stringliteral}{'.
       Stoping'}
...
412 
413         b=1
414         \textcolor{keywordflow}{do} i=1, nxi
415             \textcolor{keywordflow}{do} j=1, nyi
416                 tempbox%pt = bbox%pt + bbox%size%x*(i-1)/nxi*ex + bbox%size%y*(j-1)/nyi*ey - bbox%pt%z*ez
417                 tempbox%size = bbox%size%x/nxi*ex + bbox%size%y/nyi*ey
418                 \textcolor{keyword}{call }dblock(b)%initialize(b, tempbox)
419                 b=b+1
420 \textcolor{keywordflow}{            end do}
421 \textcolor{keywordflow}{        end do}
422         temp(1) = nxi
423         temp(2) = nyi
424         outext=\textcolor{stringliteral}{'-->Automatic domain decomposition sucessful. Domain is '}//temp(1)// \textcolor{stringliteral}{' X '} //temp(2)//\textcolor{stringliteral}{'
       Blocks'}
425         \textcolor{keyword}{call }log%put(outext,.false.)
426 \textcolor{keywordflow}{    end if}
427     globals%SimDefs%blocksize = dblock(1)%extents%size
428     \textcolor{comment}{!do i=1, size(DBlock)}
429     \textcolor{comment}{!    call DBlock(i)%print()}
430     \textcolor{comment}{!enddo}
431 
