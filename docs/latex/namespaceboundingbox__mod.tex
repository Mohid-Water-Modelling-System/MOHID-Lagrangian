...
49     self%offset = -self%pt \textcolor{comment}{!distance to the origin - local reference}
\end{DoxyCode}
...
63     temp\_str(1)=self%pt%x
64     temp\_str(2)=self%pt%y
65     temp\_str(3)=self%pt%z
...
67     temp\_str(1)=self%size%x
68     temp\_str(2)=self%size%y
69     temp\_str(3)=self%size%z
...
71     \textcolor{keyword}{call }log%put(outext,.false.)
\end{DoxyCode}
