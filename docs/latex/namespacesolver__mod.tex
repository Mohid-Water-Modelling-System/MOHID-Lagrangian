...
188     self%solverType = flag
189     self%name = name
190     \textcolor{keyword}{call }self%Kernel%initialize()
\end{DoxyCode}
...
201     outext = \textcolor{stringliteral}{'Solver algorithm is '}//self%name
...
148         \textcolor{keyword}{call }sv(i)%copyState(intsv)
149         \textcolor{keyword}{allocate}(k1(\textcolor{keyword}{size}(sv(i)%state, 1), \textcolor{keyword}{size}(sv(i)%state, 2)))
150         \textcolor{keyword}{allocate}(k2(\textcolor{keyword}{size}(sv(i)%state, 1), \textcolor{keyword}{size}(sv(i)%state, 2)))
151         \textcolor{keyword}{allocate}(k3(\textcolor{keyword}{size}(sv(i)%state, 1), \textcolor{keyword}{size}(sv(i)%state, 2)))
152         \textcolor{keyword}{allocate}(k4(\textcolor{keyword}{size}(sv(i)%state, 1), \textcolor{keyword}{size}(sv(i)%state, 2)))
153         \textcolor{comment}{!1st step}
154         k1 = self%Kernel%run(sv(i), bdata, time, dt)*dt
155         intsv%state = sv(i)%state + k1*0.5
156         \textcolor{comment}{!2nd step}
157         k2 = self%Kernel%run(intsv, bdata, time + 0.5*dt, dt)*dt
158         intsv%state = sv(i)%state + k2*0.5
159         \textcolor{comment}{!3rd step}
160         k3 = self%Kernel%run(intsv, bdata, time + 0.5*dt, dt)*dt
161         intsv%state = sv(i)%state + k3
162         \textcolor{comment}{!4th step}
163         k4 = self%Kernel%run(intsv, bdata, time + dt, dt)*dt
164         \textcolor{comment}{!computing the new state}
165         sv(i)%state = sv(i)%state + (k1 + 2.0*k2 + 2.0*k3 + k4)/6.0
166         \textcolor{comment}{!deallocating}
167         \textcolor{keyword}{call }intsv%finalize()
