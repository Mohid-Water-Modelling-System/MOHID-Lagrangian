...
114     date = date%now()
115     timestamp = date%isoformat(\textcolor{stringliteral}{' '})
116     timestamp = timestamp%basename(strip\_last\_extension=.true.) \textcolor{comment}{!getting rid of miliseconds}
\end{DoxyCode}
...
\end{center}
\end{figure}
\mbox{\Hypertarget{namespacesimulationlogger__mod_a7f4423ce42b0a9717110bd78028cec45}\label{namespacesimulationlogger__mod_a7f4423ce42b0a9717110bd78028cec45}} 
...
64     self%log\_unit = 0
65     \textcolor{keyword}{open} (unit=self%log\_unit,file=logfile%chars(),action=\textcolor{stringliteral}{"write"},status=\textcolor{stringliteral}{"replace"})
66 
\end{DoxyCode}
...
100     \textcolor{keyword}{write}(self%log\_unit,\textcolor{stringliteral}{"(A)"}) tologstr%chars()
101     print\textcolor{stringliteral}{'(A)'}, tologstr%chars()
102 
\end{DoxyCode}
